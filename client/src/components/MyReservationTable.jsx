	...
	const filterData = () => {
		...
		if (statusOption !== '전체') {
			filteredData = filteredData.filter(
				(item) => item.reserve.status === statusOption,
			);
		}
			...
			filteredData = filteredData.filter(
				(item) =>
					Moment().subtract('3', 'months').format('YYYY-MM-DD') <
						item.info.date && item.info.date <= today,
			);
						...
						<StyledList key={index}>
							<StyledListInner>
								...
								<StyledContent>
									...
									<p>
										인원 <span>{reserve.personnel}</span>명
									</p>
									<p>
										결제금액 <span>{reserve.total_price.toLocaleString()}</span>
										원
									</p>
								</StyledContent>
							</StyledListInner>
					...
					return (
						<div key={`${dataIndex}` - `${info.id}`}>
							...
							<StyledContent>
								...
								<p>
									체험 인원 <span>{reserve.personnel}명</span>
								</p>
							</StyledContent>
							<br />
								...
								<ModalContainer w="320px" h="170px">
									<StyledConfirmModal>
										...
										<StyledCancleButtonWrap marginTop>
											<DeleteButton
												name={reserve.id}
												onClick={(e) => cancleResevationHandler(e)}
											>
												확인
											</DeleteButton>
											<ConfirmButton
